...
И если про каждый из этих подходов уже было сказано немало, я хочу поговорить про то, как их совмещать в разработке.
...
Теперь при передаче свойства `modifier` или `hidden` в наши атомарные компоненты для них будет сгенерирован соответствующий класс.
